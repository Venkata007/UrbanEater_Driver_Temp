...
class OTPViewController: UIViewController,OTPTextFieldDelegate {
    ...
    func updateNewPasswordApiMethod(){
        if validatePasswords(){
            ...
            let param = [ "mobileId": TheGlobalPoolManager.updatePasswordModel.data.subId!,
                          "referenceNumber": TheGlobalPoolManager.updatePasswordModel.data.referenceNumber!,
                          "otp": validateOTP().1,
                          "password" : newPasswordTF.text!,
                          "through": "MOBILE"] as [String : Any]
    ...
    @IBAction func verifyBtn(_ sender: UIButton) {
        ...
        if validateOTP().1.length == 4{
            print(validateOTP().1)
            if self.validateOTP().1 == TheGlobalPoolManager.updatePasswordModel.data.otp!{
                self.otpBGView.isHidden = true
                self.newPasswordsView.isHidden = false
            }else{
                Theme.sharedInstance.showErrorpopup(Msg: "Entered wrong OTP")
            }
        }else{
...
extension OTPViewController{
    ...
    func validatePasswords() -> Bool{
        if (self.newPasswordTF.text?.isEmpty)!{
            ...
            Theme.sharedInstance.showErrorpopup(Msg: ToastMessages.Password_Missmatch)
            return false
        }
        return true
    }
